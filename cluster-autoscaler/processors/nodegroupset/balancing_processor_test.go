...
func TestAverageNodegroupCapacitiesBalancesEmpty(t *testing.T) {
	processor := BalancingNodeGroupSetProcessor{}
	avergages := processor.averageNodegroupCapacities([]cloudprovider.NodeGroup{}, []*apiv1.Node{})
	assert.Equal(t, map[string]int{}, avergages)
}

func TestAverageNodegroupCapacitiesFallsBackTo1WhenNoNodesWereFound(t *testing.T) {
	provider := testprovider.NewTestCloudProvider(nil, nil)
	...
	processor := BalancingNodeGroupSetProcessor{}
	avergages := processor.averageNodegroupCapacities(nodeGroups, []*apiv1.Node{})
	assert.Equal(t, map[string]int{"ng1": 1}, avergages)
}

func TestAverageNodegroupCapacitiesFillsAverages(t *testing.T) {
	provider := testprovider.NewTestCloudProvider(nil, nil)
	...
	processor := BalancingNodeGroupSetProcessor{}
	avergages := processor.averageNodegroupCapacities(nodegroups, nodes)
	assert.Equal(t, map[string]int{"ng1": 2500}, avergages)
}

func TestAverageNodegroupCapacitiesFallsBackEmptyNodegroupToOtherAverage(t *testing.T) {
	provider := testprovider.NewTestCloudProvider(nil, nil)
	...
	processor := BalancingNodeGroupSetProcessor{}
	avergages := processor.averageNodegroupCapacities(nodegroups, nodes)
	assert.Equal(t, map[string]int{"ng1": 2500, "ng2": 2500}, avergages)
}
