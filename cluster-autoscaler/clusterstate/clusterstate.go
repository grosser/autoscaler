...
// for some time.
func (csr *ClusterStateRegistry) RegisterFailedScaleUp(nodeGroup cloudprovider.NodeGroup, reason string, errorMessage, gpuResourceName, gpuType string, currentTime time.Time, backoff bool) {
	csr.Lock()
	defer csr.Unlock()
	...
	}, gpuResourceName, gpuType, currentTime, backoff)
}
